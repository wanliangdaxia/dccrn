...
class ComplexConv2d(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = in_channels // 2     #//表示整数除法,返回不大于结果的一个最大的整数
        ...
        nn.init.normal_(self.real_conv.weight.data, std=0.05)       #用正态分布的值填充输入张量
        nn.init.normal_(self.imag_conv.weight.data, std=0.05)
        nn.init.constant_(self.real_conv.bias, 0.)                  #用常数填充输入张量
    ...
    def forward(self, inputs):
        ...
        if self.complex_axis == 0:
            ...
            if isinstance(inputs, torch.Tensor):
                real, imag = torch.chunk(inputs, 2, self.complex_axis)  ##沿第几个维度切块  2块
            #print("1",real.shape)
            real2real = self.real_conv(real, )
            #print("2",real2real.shape)
